    ...
    <div
      ...
      {!hasSubMenu && (
        <Link
          target={link?.target}
          href={link?.href}
          className=' hover:bg-black hover:bg-opacity-10 rounded-3xl font-bold flex justify-center items-center px-3 py-1 no-underline tracking-widest'>
      ...
      {hasSubMenu && (
        <>
          <div className='cursor-pointer hover:bg-black hover:bg-opacity-10 rounded-3xl font-bold flex justify-center items-center px-3 py-1 no-underline tracking-widest relative'>
            {link?.icon && <i className={link?.icon} />} {link?.name}
            {/* 主菜单下方的安全区域 */}
            {show && (
